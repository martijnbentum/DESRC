...
def load_word_epochs_all_participant(participants, unload_eeg = True):
	...
	for participant in participants:
		print(participants.pp_id)
		load_word_epochs_participant(participant, unload_eeg = unload_eeg)

def load_word_epochs_participant(participant, unload_eeg = True,value_dict = {},count_dict ={}):
	...
	for i,block in enumerate(participant.blocks):
		...
		if not succes: participant.bad_blocks.append(block)
			...
			value_dict, count_dict = block2average(block,value_dict,count_dict)
		if unload_eeg: unload_eeg_data(block)
	print('pp_id:',participant.pp_id,'bad blocks:',len(participant.bad_blocks))
...
def make_eeg_word_epochs(block, channel_set = [],remove_bad_ch = True,bad_block_check = True):
	'''Extract EEG data corresponding to words in the block.
	Only words are loaded that do not overlap with artefacts in the EEG data or other words
	EEG data is loaded as muVolts
	bandpass filter 0.05 - 30 iir filter
	ica decomposition to remove EOG activity
	channel_set 		set of channels to extract default = all eeg channels
	remove_bad_ch 		whether to remove channels with poor EEG data
	bad_block_check 	whether to make word epochs if the block contains bad EEG data
						defaults to not loading EEG data when quality is poor
	'''
	if not hasattr(block,'words'): print('words are missing load blocks with add_words = True')
	block.extracted_eeg_words = []
	block.extracted_word_indices = []
	block.bad_words = []
	if bad_block_check and block.usability in ['doubtfull','bad']:
		print('skipping block because it is bad, force load by setting bad_block_check = False')
	...
	if not block.ica_applied:
		...
		return
	rbch = remove_bad_ch
	temp = raw2np(block.raw, keep_channels = channel_set, remove_bad_ch = rbch)
	block.data,block.ch,block.removed_ch = temp
	...
	print('extracted:',len(block.extracted_eeg_words),'excluded:',len(block.bad_words))
	return True
...
def extract_word_eeg_data(data, word, block_st_sample,epoch = [-300,1000], threshold = 75):
...
def raw2np(raw, keep_channels = 'all', remove_bad_ch = True):
	...
	'''
	if keep_channels == 'all': keep_channels = load_channel_set()
	elif keep_channels == 'default': keep_channels = load_channel_set('default')
	elif type(keep_channels) == list: pass
	else: raise ValueError('provide channel set type (all/default) or a list of channels to keep')
	data = raw[:][0] * 10 ** 6
...
def load_channel_set(set_type = 'all'):
	...
	if set_type == 'all':
		...
		ch = open('EEG_DATA_ifadv_cgn/channel_set_default.txt').read().split('\n')[:-1]
		assert len(ch) == 25
	else: print('unknown set type',set_type)
	return ch


def add_eeg_data(block, sf = 1000, freq = [0.05,30], apply_ica = True, remove_bad_ch = True):
	'''Load the eeg data corresponding to a block in the experiment.
	block 		block object created with read_xml
	sf 			sample frequency, lower number to downsample the sf (recorded with sf = 1000)
...
def load_eeg(vhdr_fn = None, block = None,preload = False):
	...
	if all the data of a session is loaded extreme values are loaded, because
	recording continued during breaks, this has an adverse effect on filtering.
...
def filter_iir(raw,order = 5,freq = [0.05,30],sf = 1000,pass_type = 'bandpass', verbose = False):
...
def check_word_usability(block,word):
	...
	word 		object with word info (should be from block) made with read_xml
	'''
	artefacts = block2artefacts(block)
	artefact_overlap = check_overlap(word,artefacts)
	word.artefact_overlap = artefact_overlap
	if word.overlap or artefact_overlap or '*' in word.word or 'ggg' == word.word or 'xxx' == word:
		word.usable = False
		if verbose:
			print(word.__repr__(),'| word-overlap:',\
				word.overlap,'| artefact-overlap:',artefact_overlap,'|',word.word)
	else: word.usable = True

def block2artefacts(block):
	'''creates a list of tuples whereby each tuple contains the start and end sample
	of an artefact in the EEG data of a block.
	block 		object with block info made with read_xml
	'''
...
def check_overlap(word,artefacts):
	...
	artefacts 	list of tuples with start and end samples of artefacts in a block
	'''
	for a in artefacts:
		a_st, a_et = a
		artefact_overlap = compute_overlap(word.st_sample,word.et_sample,a_st,a_et)
...
def compute_overlap(start_a,end_a,start_b, end_b):
	...
	if end_a < start_a:
		raise ValueError(' function assumes increasing intervals',start_a,end_a)
	if end_b < start_b:
		raise ValueError(' function assumes increasing intervals',start_b,end_b)
	if end_b <= start_a or start_b >= end_a: return 0 # b is completely before or after a
		...
		if end_b > end_a: return end_a - start_b #b starts after start of a and ends == 
		#or after end of a
